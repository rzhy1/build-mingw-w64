...
download_sources()
{
    ...
    execute "downloading Binutils source" "" git clone --depth 1 -b "$BINUTILS_BRANCH" git://sourceware.org/git/binutils-gdb.git binutils
    execute "downloading GCC source" "" git clone --depth 1 -b "$GCC_BRANCH" https://github.com/gcc-mirror/gcc.git gcc  
    execute "downloading config.guess" "" curl -f -L -o config.guess "https://gitweb.git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.guess;hb=HEAD" || exit 1 
...
# 每个架构的基础步骤数
BASE_BUILD_STEPS=13
BUILD_STEPS=$((BASE_BUILD_STEPS * NUM_BUILDS))
THREADS_STEPS=$((THREADS_STEPS * NUM_BUILDS))

TOTAL_STEPS=$((TOTAL_STEPS + BUILD_STEPS + THREADS_STEPS))
